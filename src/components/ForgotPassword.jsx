...
//[#0096D5] [#0096D5] [#0096D5]
    return (
        <div className='bg-red-800 h-screen relative'>
            ...
            <div className='text-center bg-red-600 font-sans text-white pt-2 text-lg mb-10
        pb-2'> <svg xmlns="http://www.w3.org/2000/svg" onClick={() => navigate('/login')} fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6 absolute left-2 cursor-pointer hover:bg-white hover:stroke-black hover:rounded-full transition rounded-full ease-in-out delay-150 duration-200">
                ...
                Find Password</div>
                {/* [#d3d6fe] */}
            <div className="box mb-20 border-2 m-auto border-gray-200 bg-red-300 rounded-3xl border-solid lg:w-3/5 w-4/5 shadow-xl shadow-red-600 p-4 w-50% flex flex-col">
                <div className="no_phone mb-3 flex items-center bg-white border-2 border-gray-100 rounded-full ">
                    ...
                    <div onClick={handleMessage} className='opt w-10 bg-red-800 mr-4 text-xs cursor-pointer p-2 shadow-md rounded-2xl text-white text-center'>OTP</div>
                </div>
                ...
                <button onClick={handleReset} disabled={!(otpfield == otp && otp.length > 0 && otpfield.length > 0)} className={`${(otpfield == otp && otp.length > 0 && otpfield.length > 0) ? 'bg-red-500' : 'bg-gray-500 cursor-not-allowed'} text-white pt-1 pb-1 rounded-full text-lg`}>Confirm</button>
